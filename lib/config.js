...
/**
 * Tests to make sure there is no content in the current directory so we know if we need to install the repo
 *
 */
const testEmpty = () => {
  data = fs.readdirSync('.')
  if((data == "")||(data ==".glitch-localdev")) return(true); // not returning null or undefined, and coerces to empty string on Windows
  return(false);
}